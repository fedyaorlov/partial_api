...
# It is needed to log into a separate Instagram API and get profiles' data, conversations and messages.
...
MAX_DIALOGS = 10000  # How many maximal last conversations should be checked to find the one needed.
...
# This function returns session instance with a user authorized to Instagram.
def logined_session(login, password):
...
# This function returns dictionary with all the conversations names for a user.
def get_conversations_list(login, password):
    # Log in to Instagram and create a session with this user authorized.
    session = logined_session(login, password)
    ...
    for item in conversations:
        interlocutor = item['thread_title']
        interlocutors.append(interlocutor)

    conv_number = len(interlocutors)
    response = {'conv_number': conv_number, 'conversations': interlocutors}
