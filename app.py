...
from insta_scrape import loginning, profile_data, get_direct_messages, get_conversations_list
...
def not_found(error):
    return error_response('Page not found. Wrong URL', 404)


@app.errorhandler(500)
def server_error(error):
    return error_response("Error occurred from the server's side", 500)
...
def direct_messages():
    ...
    response = get_direct_messages(user, login, password)
    if type(response) == str:   # It means that we received a string with an error.
        return error_response(response, 400)
